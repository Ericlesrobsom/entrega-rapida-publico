...
import { Search, MessageCircle, Check, Archive, Inbox, User as UserIcon } from "lucide-react";
...
export default function Questions() {
  ...
  const handleArchive = async (questionId) => {
    try {
      await Question.update(questionId, { status: "arquivada" });
    ...
    <div className="p-6 space-y-6 bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-2">Caixa de Entrada de Dúvidas</h1>
            <p className="text-slate-600">Responda as perguntas agrupadas por cliente</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 bg-white/80 backdrop-blur-sm shadow-lg rounded-xl p-4 h-[75vh] overflow-y-auto">
            <h2 className="text-lg font-semibold text-slate-800 mb-4 px-2">Clientes com Dúvidas</h2>
                ...
                filteredCustomers.map(customer => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg transition-all duration-200 flex items-center gap-3 ${
                      selectedCustomer?.email === customer.email ? 'bg-blue-100 shadow-sm' : 'hover:bg-slate-100'
                    }`}
                  >
          ...
          <div className="lg:col-span-3 h-[75vh] overflow-y-auto pr-2">
            {!selectedCustomer ? (
              <div className="flex flex-col items-center justify-center h-full text-center text-slate-500 bg-white/50 backdrop-blur-sm rounded-xl">
              ...
              <div className="space-y-4">
                {questionsOfSelectedCustomer.length === 0 ? (
                   <div className="text-center py-12 text-slate-500 bg-white/50 backdrop-blur-sm rounded-xl">
                    ...
                    <Card key={question.id} className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
                      ...
                      <CardContent>
                        <div className="bg-slate-50 p-4 rounded-lg mb-4">
                          <p className="text-slate-800 font-medium mb-2">Pergunta do cliente:</p>
                          <p className="text-slate-700">{question.question}</p>
                        </div>

                        {question.answer && (
                          <div className="bg-blue-50 p-4 rounded-lg mb-4">
                            <p className="text-blue-800 font-medium mb-2">Sua resposta:</p>
